...
def reconstruct_stochastic(options, iterator, f_init_recon, f_next_recon, trng, worddicts_r, dst_fh):
    assert options['use_reconstructor']
    ...
    for x, y in iterator:
        ...
        for jj in xrange(x.shape[2]):
            x_current = x[:, :, jj][:, :, None]
            y_current = y[:, jj][:, None]

            # remove padding
            x_current = x_current[:,:x_mask.astype('int64')[:, jj].sum(),:]
            y_current = y_current[:y_mask.astype('int64')[:, jj].sum(),:]

            sample, score, sample_word_probs, alignment, hyp_graph = gen_sample_recon(
                                       [f_init_recon], [f_next_recon],
                                       x_current,
                                       y_current,
                                       trng=trng, k=1,
                                       maxlen=10000,
            ...
            for vv in sample[0]:
                ...
                if vv in worddicts_r[0]:
                    x_recon.append(worddicts_r[0][vv])
                else:
                    x_recon.append('UNK')
            dst_fh.write(" ".join(x_recon) + "\n")
...
def init_params(options):
    ...
    if options['use_reconstructor']:
        # init_state, init_cell
        params = get_layer_param('ff')(options, params, prefix='ff_reconstructor_state',
        ...
        # deeper layers of the reconstructor
        if options['dec_depth'] > 1:
            if options['dec_deep_context']:
                input_dim = options['dim'] * 2
            else:
                input_dim = options['dim']

            for level in range(2, options['dec_depth'] + 1):
                params = get_layer_param(options['decoder_deep'])(options, params,
                                                prefix=pp('reconstructor', level),
                                                nin=input_dim,
                                                dim=options['dim'],
                                                dimctx=options['dim'])

        # readout
        params = get_layer_param('ff')(options, params, prefix='ff_reconstructor_logit_lstm',
                                    nin=options['dim'], nout=options['dim_word'],
                                    ortho=False)
        params = get_layer_param('ff')(options, params, prefix='ff_reconstructor_logit_prev',
                                    nin=options['dim_word'],
                                    nout=options['dim_word'], ortho=False)
        params = get_layer_param('ff')(options, params, prefix='ff_reconstructor_logit_ctx',
                                    nin=options['dim'], nout=options['dim_word'],
                                    ortho=False)

        params = get_layer_param('ff')(options, params, prefix='ff_reconstructor_logit',
                                    nin=options['dim_word'],
                                    nout=options['n_words_src'],
                                    weight_matrix = not options['tie_decoder_embeddings'],
                                    followed_by_softmax=True)
    return params
...
def build_decoder(tparams, options, y, ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=False, pctx_=None, shared_vars=None):
    ...
    next_state = proj[0]
    rnn_ctx = proj[0]  # for return at end
    ...
    return logit, opt_ret, ret_state, rnn_ctx

# RNN reconstructor (Tu et al)
# TODO This is basically a copy-and-paste of build_decoder.  If this code gets
#      integrated into master, then that function should be generalized.  In
#      the meantime, keeping it separate minimizes the risk of screwing up any
#      other code and prevents problems with conflicts
def build_reconstructor(tparams, options, x, ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=False, pctx_=None, shared_vars=None):
    opt_ret = dict()
    ...
    if options['use_dropout']:
        if sampling:
            # FIXME?  What's the correct dropout option here?
            source_dropout = dropout(dropout_probability=options['dropout_target'])
        else:
            n_timesteps = x.shape[1]
            n_samples = x.shape[2]
            # FIXME?  What's the correct dropout option here?
            source_dropout = dropout((n_timesteps, n_samples, 1), options['dropout_target'])
            source_dropout = tensor.tile(source_dropout, (1, 1, options['dim_word']))

    # word embedding (source), we will shift the source sequence one time step
    # to the right. This is done because of the bi-gram connections in the
    # readout and decoder rnn. The first target will be all zeros and we will
    # not condition on the last output.
    emb = get_layer_constr('embedding')(tparams, x, suffix='', factors=options['factors'])
    ...
    # reconstructor - pass through the decoder conditional gru with attention
    proj = get_layer_constr(options['decoder'])(tparams, emb, options, dropout,
                                            prefix='reconstructor',
    ...
    # FIXME reconstructor depth is set by dec_depth - separate option?
    ...
    logit_lstm = get_layer_constr('ff')(tparams, next_state, options, dropout,
                                    dropout_probability=options['dropout_hidden'],
                                    prefix='ff_reconstructor_logit_lstm', activ='linear')
    logit_prev = get_layer_constr('ff')(tparams, emb, options, dropout,
                                    dropout_probability=options['dropout_embedding'],
                                    prefix='ff_reconstructor_logit_prev', activ='linear')
    logit_ctx = get_layer_constr('ff')(tparams, ctxs, options, dropout,
                                   dropout_probability=options['dropout_hidden'],
                                   prefix='ff_reconstructor_logit_ctx', activ='linear')
    logit = tensor.tanh(logit_lstm+logit_prev+logit_ctx)

    # last layer
    logit_W = tparams['Wemb'].T if options['tie_decoder_embeddings'] else None
    logit = get_layer_constr('ff')(tparams, logit, options, dropout,
                            dropout_probability=options['dropout_hidden'],
                            prefix='ff_reconstructor_logit', activ='linear', W=logit_W, followed_by_softmax=True)

    return logit, opt_ret, ret_state


def build_reconstruction_cost_func(tparams, options, use_noise, trng):
    assert options['use_reconstructor']

    x = tensor.tensor3('x', dtype='int64')
    ctx = tensor.tensor3('ctx', dtype=floatX)
    ...
    init_state = get_layer_constr('ff')(tparams, ctx_mean, options, dropout,
                                        dropout_probability=options['dropout_hidden'],
                                        prefix='ff_reconstructor_state', activ='tanh')
    ...
    logit, opt_ret, _ = build_reconstructor(tparams, options, x, ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=False)
    ...
    rcost = -tensor.log(probs.flatten()[x_flat_idx])
    rcost = rcost.reshape([x.shape[1], x.shape[2]])
    rcost = rcost.sum(0)

    print >>sys.stderr, 'Building f_reconstruction_cost..',
    inps = [x, ctx]
    outs = [rcost]
    f_reconstruction_cost = theano.function(inps, outs, name='f_reconstruction_cost', profile=profile)
    print >>sys.stderr, 'Done'

    return f_reconstruction_cost
...
def build_model(tparams, options):
    ...
    logit, opt_ret, _, rnn_ctx = build_decoder(tparams, options, y, ctx, init_state, dropout, x_mask=x_mask, y_mask=y_mask, sampling=False)
    ...
    lcost = -tensor.log(probs.flatten()[y_flat_idx])
    lcost = lcost.reshape([y.shape[0], y.shape[1]])
    lcost = (lcost * y_mask).sum(0)

    if options['use_reconstructor']:
        # mean of the context (across time) will be used to initialize decoder rnn
        ctx_mean = (rnn_ctx * y_mask[:, :, None]).sum(0) / y_mask.sum(0)[:, None]
        ...
        logit, opt_ret, _ = build_reconstructor(tparams, options, x, rnn_ctx, init_state, dropout, x_mask=x_mask, y_mask=y_mask, sampling=False)
        ...
        rcost = -tensor.log(probs.flatten()[x_flat_idx])
        rcost = rcost.reshape([x.shape[1], x.shape[2]])
        rcost = (rcost * x_mask).sum(0)

        cost = lcost + options['reconstructor_hyper'] * rcost
    else:
        cost = lcost
        rcost = tensor.zeros_like(lcost)

    #print "Print out in build_model()"
    #print opt_ret
    return trng, use_noise, x, x_mask, y, y_mask, opt_ret, cost, lcost, rcost
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    logit, opt_ret, ret_state, rnn_ctx = build_decoder(tparams, options, y, ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=True)
...
# build a sampler (reconstruction)
def build_sampler_recon(tparams, options, use_noise, trng, return_alignment=False):
    ...
    logit, opt_ret, _, rnn_ctx = build_decoder(tparams, options, y, ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=False)

    ctx_mean = rnn_ctx.mean(0)

    init_state = get_layer_constr('ff')(tparams, ctx_mean, options, dropout,
                                        dropout_probability=options['dropout_hidden'],
                                        prefix='ff_reconstructor_state', activ='tanh')

    # every reconstructor RNN layer gets its own copy of the init state
    init_state = init_state.reshape([1, init_state.shape[0], init_state.shape[1]])
    if options['dec_depth'] > 1:
        init_state = tensor.tile(init_state, (options['dec_depth'], 1, 1))

    print >>sys.stderr, 'Building f_init_recon...',
    outs = [init_state, rnn_ctx]
    f_init_recon = theano.function([x, y], outs, name='f_init_recon', profile=profile)
    print >>sys.stderr, 'Done'

    # x: 1 x 1
    x_recon = tensor.vector('x_sampler', dtype='int64')
    init_state = tensor.tensor3('init_state', dtype=floatX)

    logit, opt_ret, ret_state = build_reconstructor(tparams, options, x_recon, rnn_ctx, init_state, dropout, x_mask=None, y_mask=None, sampling=True)
    ...
    # compile a function to do the whole thing above, next word probability,
    # sampled word for the next target, next hidden state to be used
    print >>sys.stderr, 'Building f_next_recon..',
    inps = [x_recon, rnn_ctx, init_state]
    outs = [next_probs, next_sample, ret_state]

    if return_alignment:
        outs.append(opt_ret['dec_alphas'])

    f_next_recon = theano.function(inps, outs, name='f_next_recon', profile=profile)
    print >>sys.stderr, 'Done'

    return f_init_recon, f_next_recon
...
def build_full_sampler(tparams, options, use_noise, trng, greedy=False):
    ...
    def decoder_step(y, init_state, ctx, pctx_, *shared_vars):
        ...
        logit, opt_ret, ret_state, rnn_ctx = build_decoder(tparams, options, y, ctx, init_state, dropout, x_mask=x_mask, y_mask=None, sampling=True, pctx_=pctx_, shared_vars=shared_vars)
...
def gen_sample(f_init, f_next, x, trng=None, k=1, maxlen=30,
               ...
               return_hyp_graph=False, fs_reconstruction_cost=None, word_idict_trg=None):
    ...
    hyp_states = []
    hyp_state_seqs = [[] for i in xrange(live_k)]
    if return_alignment:
    ...
    for ii in xrange(maxlen):
        ...
        if stochastic:
            ...
            for idx, [ti, wi] in enumerate(zip(trans_indices, word_indices)):
                ...
                new_hyp_state_seqs.append(hyp_state_seqs[ti] + [[copy.copy(next_state[i][ti]) for i in xrange(num_models)]])
                if return_alignment:
            ...
            for idx in xrange(len(new_hyp_samples)):
                ...
                if new_hyp_samples[idx][-1] == 0:
                    ...
                    sample_state_seqs.append(new_hyp_state_seqs[idx])
                    if return_alignment:
                    ...
                    hyp_state_seqs.append(copy.copy(new_hyp_state_seqs[idx]))
    ...
    if not argmax and live_k > 0:
        for idx in xrange(live_k):
            ...
            sample_state_seqs.append(hyp_state_seqs[idx])
            if return_alignment:
    ...
    if fs_reconstruction_cost:
        ...
        for i, state_seq in enumerate(sample_state_seqs):
            r_cost_sum = 0
            for j in xrange(num_models):
                # get the decoder state sequence s for model j for this hypothesis
                ctx = numpy.array([s[j] for s in state_seq])
                #ctx = numpy.transpose(ctx, (1, 0, 2))
                #ForkedPdb().set_trace()
                outs = fs_reconstruction_cost[j](x, ctx)
                r_cost_sum += outs[0][0]
            avg_r_cost = r_cost_sum / num_models
            lcost = sample_score[i]
            # FIXME lambda
            sample_score[i] += avg_r_cost
            print "DEBUG: %d %d %f %f %s" % (i, x.shape[1], lcost, avg_r_cost, _seqs2words(sample[i]))
        print "DEBUG:"
        # TODO reorder alignment? hyp_graph?

    return sample, sample_score, sample_word_probs, alignment, hyp_graph

# generate reconstruction sample with stochastic sampling (beam search to be added later)
# this function iteratively calls f_init_recon and f_next_recon functions.
def gen_sample_recon(f_init_recon, f_next_recon, x, y, trng=None, k=1, maxlen=30,
               stochastic=True, argmax=False, return_alignment=False, suppress_unk=False,
               return_hyp_graph=False):
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True, normalization_alpha=0.0, alignweights=False):
    out_costs = []    # Total cost
    out_tcosts = []   # Translation cost
    out_rcosts = []   # Reconstruction cost
    n_done = 0
    ...
    for x, y in iterator:
        ...
        if alignweights:
            cost, tcost, rcost, attention = f_log_probs(x, x_mask, y, y_mask)
            for jdata in get_alignments(attention, x_mask, y_mask):
                alignments_json.append(jdata)
        else:
            cost, tcost, rcost = f_log_probs(x, x_mask, y, y_mask)

        # normalize translation costs according to output length
        if normalization_alpha:
            adjusted_lengths = numpy.array([numpy.count_nonzero(s) ** normalization_alpha for s in y_mask.T])
            tcost /= adjusted_lengths

        # normalize reconstruction costs according to input length
        if normalization_alpha:
            adjusted_lengths = numpy.array([numpy.count_nonzero(s) ** normalization_alpha for s in x_mask.T])
            rcost /= adjusted_lengths

        # combine translation and reconstruction costs and add all costs to output lists
        for (t, r) in zip(tcost, rcost):
            out_costs.append(t + options['reconstructor_hyper'] * r)
            out_tcosts.append(t)
            out_rcosts.append(r)

        logging.debug('%d samples computed' % (n_done))

    return numpy.array(out_costs), numpy.array(out_tcosts), numpy.array(out_rcosts), alignments_json
...
def train(dim_word=512,  # word vector dimensionality
          ...
          use_reconstructor=False, # use reconstructor (Tu et al., 2017)
          reconstructor_hyper=1.0, # weight for reconstruction cost
    ...
    if valid_datasets and validFreq:
        valid = TextIterator(valid_datasets[0], valid_datasets[1],
                            ...
                            use_factor=(factors>1),
                            maxlen=maxlen)
        valid2 = TextIterator(valid_datasets[0], valid_datasets[1],
                            dictionaries[:-1], dictionaries[-1],
                            n_words_source=n_words_src, n_words_target=n_words,
                            batch_size=valid_batch_size,
                            maxlen=10000,sort_by_length=False)
    else:
        valid = None
    ...
    if validFreq or sampleFreq:
        ...
        if model_options['use_reconstructor']:
            print 'Building sampler (reconstruction)'
            f_init_recon, f_next_recon = build_sampler_recon(tparams, model_options, use_noise, trng)
    if model_options['objective'] == 'MRT':
    ...
    f_log_probs = theano.function(inps, [cost, lcost, rcost], profile=profile)
    logging.info('Done')
    ...
    for training_progress.eidx in xrange(training_progress.eidx, max_epochs):
        ...
        for x, y in train:
            ...
            if valid is not None and validFreq and numpy.mod(training_progress.uidx, validFreq) == 0:
                use_noise.set_value(0.)
                valid_errs, valid_tcost, valid_rcost, alignment = pred_probs(f_log_probs, prepare_data,
                                        model_options, valid)
                ...
                logging.info('Valid {}'.format(valid_err))
                if model_options['use_reconstructor']:
                    logging.info('Valid (likelihood) {}'.format(valid_tcost.mean()))
                    logging.info('Valid (reconstruction) {}'.format(valid_rcost.mean()))

                if external_validation_script and numpy.mod(training_progress.uidx, validFreq2) == 0:
            ...
            if model_options['use_reconstructor'] and valid is not None and valid_reconstruction_freq and numpy.mod(training_progress.uidx, valid_reconstruction_freq) == 0:
                # Stochastically reconstruct source sentences and write to file
                dst_path = '{}.recon{}'.format(os.path.splitext(saveto)[0], training_progress.uidx)
                with open(dst_path, "w") as dst_fh:
                    reconstruct_stochastic(model_options, valid2, f_init_recon, f_next_recon, trng, worddicts_r, dst_fh)
...
if __name__ == '__main__':
    ...
    network.add_argument('--use_reconstructor', action="store_true",
                         help="use reconstructor (default: %(default)s)")
    network.add_argument('--reconstructor_hyper', type=float, default=1.0, metavar="FLOAT",
                         help="hyperparameter for reconstructor (default: %(default)s)")
    ...
    validation.add_argument('--validFreq2', type=int, default=10000, metavar='INT',
                         help="validation frequency (default: %(default)s)")
    validation.add_argument('--valid_reconstruction_freq', type=int, default=10000, metavar='INT',
                         help="validation reconstruction frequency (default: %(default)s)")
